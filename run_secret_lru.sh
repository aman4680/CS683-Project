...
CHAMPSIM_BIN="./bin/champsim-lru"
TRACES_DIR="../traces/secret_traces"
OUTPUT_DIR="./results/lru_policy/secret_traces/compute_fp"
...
for set in "${TRACE_SETS[@]}"; do
    ...
    for trace_file in "$current_trace_dir"/*.champsimtrace.xz; do
        ...
        if [ -f "$trace_file" ]; then
            ...
            "$CHAMPSIM_BIN" --warmup-instructions $WARMUP_INST --simulation-instructions $SIM_INST "$trace_file" > "$output_file" 2>&1
