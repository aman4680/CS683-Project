...
def parse_champsim_output(file_path):
    ...
    l2c_total_miss = get_value(r"cpu0_L2C TOTAL\s+ACCESS:.*MISS:\s+(\d+)", content, value_type=int)
    l2c_load_miss = get_value(r"cpu0_L2C LOAD\s+ACCESS:.*MISS:\s+(\d+)", content, value_type=int)
    l1d_total_miss = get_value(r"cpu0_L1D TOTAL\s+ACCESS:.*MISS:\s+(\d+)", content, value_type=int)
    l1d_load_miss = get_value(r"cpu0_L1D LOAD\s+ACCESS:.*MISS:\s+(\d+)", content, value_type=int)
    l1i_total_miss = get_value(r"cpu0_L1I TOTAL\s+ACCESS:.*MISS:\s+(\d+)", content, value_type=int)
    l1i_load_miss = get_value(r"cpu0_L1I LOAD\s+ACCESS:.*MISS:\s+(\d+)", content, value_type=int)

    # --- Average Miss Latencies ---
    llc_latency = get_value(r"LLC AVERAGE MISS LATENCY: (\d+(?:\.\d+)?)", content)
    l2c_latency = get_value(r"cpu0_L2C AVERAGE MISS LATENCY: (\d+(?:\.\d+)?)", content)
    l1d_latency = get_value(r"cpu0_L1D AVERAGE MISS LATENCY: (\d+(?:\.\d+)?)", content)
    l1i_latency = get_value(r"cpu0_L1I AVERAGE MISS LATENCY: (\d+(?:\.\d+)?)", content)
...
if __name__ == "__main__":
    ...
    for filename in sorted(os.listdir(results_dir)):
        # You can add more file extensions if needed, e.g., ('.txt', '.log', '.out')
    ...
    if all_data:
        ...
        column_order = [
            "Trace", "LRU IPC", "LLC Total MPKI", "LLC Load MPKI", "LLC Latency",
        ...
        print("No data was parsed. The CSV file was not created.")