...
# OUTPUT_DIR="./results/lru_policy"
# POLICY_TYPE="LRU"

# === BLOCK 2: Configuration for PACIPV ===
CHAMPSIM_BIN="./bin/champsim_test_PACIPV"
OUTPUT_DIR="./results/pacipv_policy/secret_traces"
POLICY_TYPE="PACIPV"
...
while IFS= read -r trace_filename || [[ -n "$trace_filename" ]]; do
    ...
    if [ -f "$full_trace_path" ]; then
        ...
        if [ "$POLICY_TYPE" == "LRU" ]; then
            ...
            echo "This is for LRU block"
        elif [ "$POLICY_TYPE" == "PACIPV" ]; then
            # echo "env L1I_IPV=\"1_2_2_1_4#1_2_1_1_4\" L1D_IPV=\"1_2_2_1_4#1_2_1_1_4\" L2C_IPV=\"1_2_2_1_4#1_2_1_1_4\" LLC_IPV=\"1_2_2_1_4#1_2_1_1_4\" $CHAMPSIM_BIN --warmup-instructions $WARMUP_INST --simulation-instructions $SIM_INST "$full_trace_path" > "$output_file" 2>&1"
            env \
            ...
            "$full_trace_path" > "$output_file" 2>&1
        fi
